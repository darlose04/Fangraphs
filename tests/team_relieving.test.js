...
describe("test route for returning all team relieving pitching statistics", () => {
  test("all team relieving pitching stats are returned as json", async () => {
    console.log("Entered test...");
    await api
      .get("/api/teamrelieving")
      .expect(200)
      .expect("Content-Type", /application\/json/);
  });

  test("there are 540 entries returned", async () => {
    const response = await api.get("/api/teamrelieving");
...
describe("test route for returning team relieving pitching stats by season", () => {
  test("check that each season is returned as json", async () => {
    let year = 2002;
    while (year < 2020) {
      await api
        .get(`/api/teamrelieving/${year}`)
        .expect(200)
        .expect("Content-Type", /application\/json/);
  ...
  test("make sure that 30 teams are being returned", async () => {
    let year = 2002;
    while (year < 2020) {
      let response = await api.get(`/api/teamrelieving/${year}`);
...
describe("test route that gets relieving pitching stats by team", () => {
  test("check that team stats are returned as json", async () => {
    let team = "Angels";
    await api
      .get(`/api/teamrelieving/teams/${team}`)
      .expect(200)
      .expect("Content-Type", /application\/json/);
  });
  ...
  test("check that team stats are ordered by season", async () => {
    let team = "Red Sox";
    let response = await api.get(`/api/teamrelieving/teams/${team}`);
